...
class _CleanerProfilePageState extends State<CleanerProfilePage> {
  ...
  Future<void> _loadRating() async {
    ...
    try {
      ...
      final averageRating = await reviewsRepo.getAverageRatingForCleaner(
        _cleanerId!,
      );
      final reviewCount = await reviewsRepo.getReviewCountForCleaner(
        _cleanerId!,
      );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: widget.isOwnProfile
          ? null
          : (widget.hideBars ? _buildAgencyAppBar() : _buildClientAppBar()),
      ...
      bottomNavigationBar: (widget.isOwnProfile || widget.hideBars)
          ? null
          : BottomNavigationBar(
  ...
  Widget _buildHistoryTab() {
    if (_cleanerId == null) {
      return Padding(
        padding: const EdgeInsets.all(16),
        child: Center(
          child: Text(AppLocalizations.of(context)!.cleanerIdNotFound),
        ),
      );
    }
  ...
  Widget _buildServiceItem({required IconData icon, required String service}) {
    return Container(
      padding: const EdgeInsets.all(16),
      decoration: BoxDecoration(
        color: Colors.white,
  ...
  Widget _buildServicesList() {
    ...
    IconData getServiceIcon(String service) {
      ...
      if (lowerService.contains('home') ||
          lowerService.contains('residential')) {
        return Icons.home_outlined;
      } else if (lowerService.contains('office')) {
        return Icons.business_outlined;
      } else if (lowerService.contains('industrial')) {
        return Icons.factory_outlined;
      } else if (lowerService.contains('specialty') ||
          lowerService.contains('special')) {
