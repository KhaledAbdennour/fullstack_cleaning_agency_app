...
class ReviewsDB extends AbstractReviewsRepo {
  ...
  Future<Review> addReview({
    ...
    try {
      DebugLogger.log('ReviewsDB', 'addReview_START', data: {
        'jobId': jobId,
      ...
      DebugLogger.log('ReviewsDB', 'addReview_JOB_LOADED', data: {
        'jobId': jobId,
      ...
      if (job.status != JobStatus.completed) {
        throw Exception('Reviews can only be added for completed jobs. Current job status: ${job.status.name}');
      }
      ...
      final reviewerProfile = await profilesRepo.getProfileById(currentUserIdInt);
      ...
      final reviewerId = currentUserIdInt.toString(); // Use string ID for consistency

      // Determine reviewee role from job context
      String revieweeRole = 'Individual Cleaner';
      if (job.assignedWorkerId != null) {
        final workerProfile = await profilesRepo.getProfileById(job.assignedWorkerId!);
        if (workerProfile != null) {
          revieweeRole = readString(workerProfile['user_type']) ?? 'Individual Cleaner';
        }
      }

      DebugLogger.log('ReviewsDB', 'addReview_USER_DATA', data: {
      ...
      DebugLogger.log('ReviewsDB', 'addReview_BEFORE_WRITE', data: {
      ...
      final reviewData = {
        ...
        'created_at_ms': DateTime.now().millisecondsSinceEpoch, // For stable sorting
      };

      // Write review document to NEW "reviews" collection (NO transaction for simple write)
      await reviewRef.set(reviewData);

      DebugLogger.log('ReviewsDB', 'addReview_REVIEW_SAVED', data: {
        'reviewDocId': reviewDocId,
        'jobId': jobId,
        'collection': 'reviews',
      });
      ...
      try {
        ...
        final reviewerName = readString(reviewerProfile['full_name']) ?? 'Anonymous';
        ...
        final oldReviewData = {
          ...
          'comment': comment,
          'date': now.toIso8601String(), // String ISO - matches query orderBy('date')
          'has_photos': 0,
          'photo_urls': null,
          'created_at': FieldValue.serverTimestamp(), // Also store Timestamp for future use
          'created_at_ms': now.millisecondsSinceEpoch, // Int for stable sorting fallback
        };

        await oldReviewRef.set(oldReviewData);

        DebugLogger.log('ReviewsDB', 'addReview_OLD_COLLECTION_SAVED', data: {
        ...
        DebugLogger.error('ReviewsDB', 'addReview_OLD_COLLECTION_FAILED', e, stack, data: {
          'revieweeId': revieweeId,
        });
      }
      ...
      try {
        ...
        if (!profileDoc.exists) {
          DebugLogger.log('ReviewsDB', 'addReview_PROFILE_NOT_FOUND', data: {
            'revieweeId': revieweeId,
        ...
        final oldRatingAvg = readDouble(profileData['rating']) ?? readDouble(profileData['rating_avg']) ?? 0.0;
        final oldRatingCount = readInt(profileData['rating_count']) ?? 0;

        DebugLogger.log('ReviewsDB', 'addReview_AGGREGATES_BEFORE', data: {
          'revieweeId': revieweeId,
          'profileDocId': profileDocId,
        ...
        final ratingsMap = <String, int>{}; // key: jobId_reviewerId, value: rating
        ...
        DebugLogger.log('ReviewsDB', 'addReview_AGGREGATES_CALCULATED', data: {
          'revieweeId': revieweeId,
        ...
        await FirebaseConfig.firestore.runTransaction((transaction) async {
          ...
          if (!profileDocInTx.exists) {
            DebugLogger.log('ReviewsDB', 'addReview_PROFILE_NOT_FOUND_IN_TX', data: {
              'profileDocId': profileDocId,
            });
          ...
          DebugLogger.log('ReviewsDB', 'addReview_AGGREGATES_UPDATED', data: {
            'revieweeId': revieweeId,
            'profileDocId': profileDocId,
            'oldRatingAvg': oldRatingAvg,
            'newRatingAvg': ratingAvg,
        ...
        DebugLogger.error('ReviewsDB', 'addReview_AGGREGATE_UPDATE_FAILED', e, stack, data: {
          'revieweeId': revieweeId,
          'profileDocId': profileDocId,
        });
      ...
      try {
        ...
        DebugLogger.error('ReviewsDB', 'addReview_NOTIFICATION_FAILED', e, StackTrace.current, data: {
          'revieweeId': revieweeId,
        });
      ...
      if (savedDoc.exists) {
        ...
        DebugLogger.log('ReviewsDB', 'addReview_SUCCESS', data: {
      ...
      DebugLogger.error('ReviewsDB', 'addReview_ERROR', e, stack, data: {
        'jobId': jobId,
        'revieweeId': revieweeId,
      });
      rethrow;
  ...
  Future<List<Review>> getReviewsForReviewee(String revieweeId) async {
    try {
      ...
      DebugLogger.error('ReviewsDB', 'getReviewsForReviewee_ERROR', e, stack, data: {
        'revieweeId': revieweeId,
      });
      ...
      try {
        ...
        reviews.sort((a, b) {
          final aMs = a.createdAtMs ?? (a.createdAt?.millisecondsSinceEpoch ?? 0);
          final bMs = b.createdAtMs ?? (b.createdAt?.millisecondsSinceEpoch ?? 0);
  ...
  Future<List<Review>> getReviewsForJob(int jobId) async {
    try {
      ...
      DebugLogger.error('ReviewsDB', 'getReviewsForJob_ERROR', e, stack, data: {
        'jobId': jobId,
      });
      return [];
    }
  ...
  Future<List<Review>> getReviewsByReviewer(String reviewerId) async {
    try {
      ...
      DebugLogger.error('ReviewsDB', 'getReviewsByReviewer_ERROR', e, stack, data: {
        'reviewerId': reviewerId,
      });
      return [];
    }
