...
class BookingsDB extends AbstractBookingsRepo {
  ...
  Future<Booking> createBooking(Booking booking) async {
    try {
      DebugLogger.log('createBooking', 'START', data: {
        'jobId': booking.jobId,
      ...
      DebugLogger.log('createBooking', 'WRITING', data: {
        'docId': docId,
        'bookingMap': sanitizedMap,
      });
      ...
      DebugLogger.log('createBooking', 'VERIFIED', data: {
        'docId': docId,
      ...
      if (booking.bidPrice != null && booking.jobId != null) {
        try {
          final jobsRepo = AbstractJobsRepo.getInstance();
          ...
          if (job != null) {
            ...
            if (currentMin == null || bidPrice < currentMin) {
              ...
              DebugLogger.log('createBooking', 'BUDGET_MIN_UPDATED', data: {
                'jobId': booking.jobId,
                'oldMin': currentMin,
                'newMin': bidPrice,
              });
          ...
          DebugLogger.log('createBooking', 'BUDGET_UPDATE_ERROR', data: {
            'error': e.toString(),
            'jobId': booking.jobId,
          });
        }
      ...
      // Notify client (async)
      if (booking.providerId != null && booking.clientId != null) {
        Future.microtask(() async {
          try {
            ...
            DebugLogger.log('createBooking', 'NOTIFICATION_ERROR', data: {'error': e.toString()});
          }
        });
      }
      
      DebugLogger.log('createBooking', 'SUCCESS', data: {'docId': docId, 'bookingId': createdBooking.id});
  ...
  Future<List<Booking>> getApplicationsForJob(int jobId) async {
    try {
      DebugLogger.log('getApplicationsForJob', 'START', data: {
        'jobId': jobId,
        'jobIdType': jobId.runtimeType.toString(),
      });
      ...
      try {
        ...
        DebugLogger.log('getApplicationsForJob', 'QUERY_SUCCESS', data: {
          'jobId': jobId,
        ...
        if (isIndexError) {
          DebugLogger.log('getApplicationsForJob', 'INDEX_MISSING_FALLBACK', data: {
            'jobId': jobId,
            'error': errorStr,
          });
          print('⚠️ [getApplicationsForJob] Index missing, using fallback query (no orderBy)');
          ...
          try {
            ...
            usedFallback = true;
            DebugLogger.log('getApplicationsForJob', 'FALLBACK_SUCCESS', data: {
              'jobId': jobId,
              'resultCount': snapshot.docs.length,
            });
          } catch (fallbackError, fallbackStack) {
            DebugLogger.error('getApplicationsForJob', 'FALLBACK_FAILED', fallbackError, fallbackStack, data: {'jobId': jobId});
            rethrow; // If fallback also fails, rethrow
          }
        } else {
          // Non-index error - rethrow immediately
          DebugLogger.error('getApplicationsForJob', 'QUERY_FAILED', e, stack, data: {
            'jobId': jobId,
      ...
      for (final doc in snapshot.docs) {
        try {
          ...
          if (data['provider_id'] == null) {
            DebugLogger.log('getApplicationsForJob', 'FILTERED_NO_PROVIDER', data: {
              'docId': doc.id,
              'jobId': jobId,
            });
            continue;
          ...
          DebugLogger.log('getApplicationsForJob', 'BOOKING_PASSED', data: {
            'docId': doc.id,
          ...
          DebugLogger.error('getApplicationsForJob', 'PARSE_ERROR', e, stack, data: {'docId': doc.id});
          continue;
        }
      }
      ...
      if (usedFallback) {
        ...
        DebugLogger.log('getApplicationsForJob', 'CLIENT_SIDE_SORT', data: {
          'jobId': jobId,
          'sortedCount': bookings.length,
        });
      }

      DebugLogger.log('getApplicationsForJob', 'SUCCESS', data: {
        'jobId': jobId,
      ...
      DebugLogger.error('getApplicationsForJob', 'ERROR', e, stacktrace, data: {'jobId': jobId});
  ...
  Future<void> acceptApplication(int bookingId) async {
    try {
      ...
      if (booking != null && booking.providerId != null) {
        ...
        if (job != null) {
          DebugLogger.log('acceptApplication', 'POST_TX_VERIFY', data: {
            'jobId': booking.jobId,
            'assignedWorkerId': job.assignedWorkerId,
            'assignedWorkerIdType': job.assignedWorkerId?.runtimeType.toString() ?? 'null',
            'providerId': booking.providerId,
  ...
  Future<void> withdrawApplication(int bookingId) async {
    try {
      ...
      if (booking.status != BookingStatus.pending) {
        throw Exception('Cannot withdraw application - status is ${booking.status.name}');
      }
          ...
          try {
            ...
            if (job != null) {
              await NotificationBackendService.sendToUser(
                ...
                body: 'A worker has withdrawn their application for "${job.title}".',
                route: '/jobDetails',
                id: booking.jobId.toString(),
              );
            }
