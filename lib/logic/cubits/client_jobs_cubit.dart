...
class ClientJobsCubit extends Cubit<ClientJobsState> {
  ...
  Future<void> loadClientJobs(int? clientId) async {
    ...
    DebugLogger.log('ClientJobsCubit', 'loadClientJobs_START', data: {
      'clientId': clientId,
    ...
    if (clientId == null) {
      ...
      DebugLogger.error('ClientJobsCubit', 'NULL_CLIENT_ID', Exception(errorMsg), StackTrace.current);
    ...
    try {
      ...
      DebugLogger.log('ClientJobsCubit', 'loadClientJobs_SUCCESS', data: {
        'clientId': clientId,
      ...
      DebugLogger.error('ClientJobsCubit', 'loadClientJobs_FAILED', e, stack, data: {
        'clientId': clientId,
      });
